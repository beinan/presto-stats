...
export default connect(mapStateToProps, { addActiveQuery, removeActiveQuery })(ChartList)
...
function extractOperatorSummary(active_queries, selected_props) {
  ...
  //list all the props in the queryStats object
  const prop_list = Object.keys(active_queries[0].jsonStats.queryStats.operatorSummaries[0])
      ...
      <Row>
        <Col xl="6">
          <QueryStatsChartWrapper
            ...
            default_props={["executionTime", "planningTime", "queuedTime", "elapsedTime"]} type="bar" />
          <QueryStatsChartWrapper sectionHeading="Tasks"
            active_queries={active_queries} extractor={extractQueryStats} type="bar"
            default_props={["totalTasks", "runningTasks", "completedTasks"]} />
          <QueryStatsChartWrapper sectionHeading="Data Size(GB)" active_queries={active_queries} extractor={extractQueryStats} type="bar" default_props={["physicalInputDataSize", "rawInputDataSize", "processedInputDataSize", "outputDataSize"]} />
        </Col>
        <Col xl="6">
          <QueryStatsChartWrapper sectionHeading="CPU Time(Second)" active_queries={active_queries} extractor={extractQueryStats} type="bar" default_props={["totalCpuTime", "totalScheduledTime", "totalBlockedTime"]} />
          <QueryStatsChartWrapper sectionHeading="Drivers" active_queries={active_queries} extractor={extractQueryStats} type="bar" default_props={["totalDrivers", "completedDrivers", "blockedDrivers", "queuedDrivers"]} />
          <QueryStatsChartWrapper sectionHeading="Memory (GB)" active_queries={active_queries} extractor={extractQueryStats} type="bar" default_props={["peakTaskTotalMemory", "peakTotalMemoryReservation", "peakTaskRevocableMemory"]} />
        </Col>
      </Row>
      <Row>
        <Col xl="12">
          <OperatorSummaryChartWraper active_queries={active_queries} default_prop="addInputWall" />
        </Col>
      </Row>
    </Fragment>
  )
...
function OperatorSummaryChartWraper(props) {
  ...
  const { options, series, prop_list } = extractOperatorSummary(active_queries, selected_props)
  ...
  const menu_items = prop_list.map((prop) => 
    (
      <div role="menuitem" key={"prop_menu" + prop} style={{ paddingLeft: 30 }}>
        <Label check>
          <Input type="checkbox" id={"checkbox_" + prop} checked={selected_props == prop}
  ...
  return (
    <div>
      <Card className="card-box mb-5">
        ...
        <CardBody><Chart options={options} series={series} type="bar" /></CardBody>
      </Card>
    </div>
  )
}
function QueryStatsChartWrapper(props) {
  ...
  const { options, series, prop_list } = extractQueryStats(active_queries, selected_props)

  const menu_items = prop_list.map((prop) => (
      <div role="menuitem" key={"prop_menu" + prop} style={{ paddingLeft: 30 }}>
        <Label check>
          <Input type="checkbox" id={"checkbox_" + prop} checked={selected_props.includes(prop)}
            onChange={e => checkboxOnChange(e, prop)} />{' '}
          {prop}
        </Label>
      </div>
  ...
  return (
    <Card className="card-box mb-5">
      ...
      <CardBody><Chart options={options} series={series} type={props.type} /></CardBody>
    </Card>
  )
}
