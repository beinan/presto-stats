...
export default connect(mapStateToProps, { addActiveQuery, removeActiveQuery })(StatsTable)

function StageSumaryTable(props) {
    const stage_id = props.stage_id
    const active_queries = props.active_queries
    const columns = ["totalDrivers", "totalCpuTime", "avgCpuTime", "totalBlockedTime", "avgBlockedTime", "totalTasks"]
    const column_names = columns.map((column) => 
        ...
        <tr>
            ...
            {columns.map((column) => {
                    if(column == "avgCpuTime" || column == "avgBlockedTime")
                        return <td>{query.jsonStats.stages[stage_id][column].toFixed(2)}ms</td>
                    else
                        return <td>{query.jsonStats.stages[stage_id][column]}</td>
                }
            )}
        </tr>
    )  
...
function StageOperatorsTable(props) {
    ...
    if(!active_queries || active_queries.length == 0)
        return  <p />
    ...
    for(let i = 0; i < total_ppls; i++) {
        ...
        for(let j = 0; j < _ppls[i].operators.length; j++) {
            operator_names.push(
                <th>{_ppls[i].operators[j].operatorType.substr(0, _ppls[i].operators[j].operatorType.length - 8)}</th>
            )
        }
    }
    ...
    active_queries.forEach(query => {
        ...
        query.jsonStats.stages[stage_id].tasks[0].pipelines.forEach(ppl => {
            _row.push(
                <td>{ppl_total_drivers[ppl.ppl_id]}</td>
            )
            ppl.operators.forEach(op => {
                _row.push(
                    <td>{(op_sumary[op.operatorType] / query.jsonStats.stages[stage_id].totalTasks).toFixed(2)}</td>
                )
            })
        })
        queries_operators.push(
            <tr>
                {_row}
            </tr>
        )
    ...
    return (
        <Table responsive size="sm" bordered>
            <thead>
                <tr>
                    <th rowspan="2">query</th>{ppl_header}
                </tr>
                <tr>
                    {operator_names}
                </tr>
