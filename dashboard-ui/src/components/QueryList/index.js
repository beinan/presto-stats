...
export default function QueryList(props) {
  ...
  const queryItems = queryList.map((query, index) => (
    <QueryItem query={query} key={index} />
  ));
  return <Fragment>{queryItems}</Fragment>;
}

const QueryItem = connect(mapStateToProps, {
  addActiveQuery,
  removeActiveQuery
})(QueryItemRender);
        ...
        <Row className="d-flex align-items-center">
          <CardBody>
            <Alert color={active_query_color}>
              <PerfectScrollbar
                style={{ height: 40, paddingTop: 2, fontSize: 10 }}>
